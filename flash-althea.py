"""
Usage:
  autoflash flash [--device=<id>] [--loop] [--tmpdir=<dir>] [--router-ip=<ip>] [-u] [-v] [--vv]
...
Options:
  ...
  -u                     Update, this device is already running and needs a update
...
def download_intermediary_firmware(tmpdir, device):
    ...
    logging.info("Downloading Intermediary firmware file " +
                 filename + " to " + tmpdir)
...
def ssh_setup(device):
    logging.info("ssh {}@{}:{}".format(
        device['sshUser'], device['sshIP'], device['sshPort']))
    ...
    while True:
        try:
            ...
            print(
                "If this happens for more than 5 minutes, double check router power and connection")
            sleep(30)
        break
...
def flash_sysupgrade(device):
    input("Plug into the Althea devices LAN port (physically furthest from the lan port) then press enter")
    client, sftp = ssh_setup(device)

    firmware_location = "/tmp/" + device['firmwareFilename']
    sftp.put(device['firmwarePath'], firmware_location)

    stdout, stderr, rc = client.exec_command(
        "sysupgrade -v -n" + firmware_location)
    print(stdout)
    print(stderr)
...
def flash_edgerouterx(device, opts):
    ...
    run_ssh_command(client, "{} add system image {}".format(
        env_magic, intermediary_firmware_location))
...
def flash_device(device, opts):
    if opts['-u']:
        # Upgrade is a special case of the sysupgrade code where we can
        # use the default values from the firmware.
        device['sshIP'] = "192.168.1.1"
        device['sshPort'] = "22"
        device['sshUser'] = "root"
        device['sshPassword'] = ""
        flash_sysupgrade(device)
    elif device['flashMethod'] == "sysupgrade":
        flash_sysupgrade(device)
        ...
        logging.error(
            "Flashing method for this device is not supported, please update this program")
        exit(1)
...
please make sure your computer has internet access until instructed to connect devices")
    ...
    if opts['list-devices']:
        ...
        device['firmwarePath'], device['firmwareFilename'] = download(
            tmpdir, device)
